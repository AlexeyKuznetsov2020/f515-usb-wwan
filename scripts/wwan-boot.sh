...
# На сколько секунд стенные часы должны обогнать длину паузы, чтобы считать, что
# голова спала. sleep считает монотонное время и во сне стоит, стенные часы идут —
# по расхождению сон и виден. Двадцати секунд достаточно: обычная погрешность
# планировщика в доли секунды, а любой реальный сон длиннее.
...
watch_sleep() {
	...
	while [ "$_ws_left" -gt 0 ]; do
		...
		_ws_t0=$(date +%s)
		sleep "$_ws_step"
		_ws_slept=$(( $(date +%s) - _ws_t0 ))
		_ws_left=$((_ws_left - _ws_step))
