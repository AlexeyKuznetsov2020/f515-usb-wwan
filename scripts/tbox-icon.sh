...
start)
	...
	# Оба пути работают одновременно, и это намеренно:
	#   SOME/IP  — штатная цепочка, по ней панель рисует иконку там, где она есть;
	#   файл     — $SIGNAL_BRIDGE, откуда сигнал берёт Frida-твик iSpaceToolbox
	#              «Статус сети»; там, где штатной иконки нет, это единственный путь.
	#
	# Файл пишется ВСЕГДА, независимо от того, заявлен ли в машине TBOX. Он лежит в
	# tmpfs, стоит одной записи в несколько секунд и ничему не мешает: кто не читает,
	# тот не замечает. Зато исчезает целый класс ошибок — «твик включили, а сигнала
	# нет», «capable ошибся», «конфиг машины не прочитался на раннем старте». Раньше
	# файл появлялся только при tbox=0, и в этих случаях иконки не было ни там ни там.
	#
	# Определение capable осталось, но теперь оно влияет только на то, что мы
	# пишем в лог: по нему видно, откуда ждать иконку на этой машине.
	if capable; then
		echo "режим: штатная цепочка ($CAPABLE_WHY) + сигнал в $SIGNAL_BRIDGE"
	else
		echo "режим: только мост в файл — $CAPABLE_WHY"
