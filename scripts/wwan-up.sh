...
if [ -z "$ADDR" ] && [ "$CHECK_ONLY" = 1 ]; then
	...
	# Пинг наружу тут не делаем намеренно. Наше дело — модемная сеть: интерфейс,
	# адрес, шлюз, маршрут. Достижимость чужого сервера от нас не зависит и
	# ничего не говорит о том, правильно ли всё настроено: в паркинге или при
	# срезанном ICMP пинг молчит на исправном модеме, и единственным следствием
	# была бы ложная тревога.
	if [ "$CHECK_ONLY" = 0 ]; then
		ok "модемная сеть настроена: $WAN_IF $ADDR через $GW"
	fi
fi
...
if [ "$CHECK_ONLY" = 0 ] && [ -n "$ADDR" ]; then
	say "   состояние: модемная сеть настроена (интернет наружу не проверяем)"
fi
say "   лог:       $LOG"
